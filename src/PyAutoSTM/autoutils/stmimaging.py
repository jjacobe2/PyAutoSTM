...
def denoise_image(img: np.ndarray, width: float, method: str = 'gaussian filter', mol_eff_size: float = CO_EFFECTIVE_SIZE):
    ...
    if method == 'gaussian filter':
        ...
        kernel_size = int(mol_eff_size/width * 256 )
...
def process_img(img: np.ndarray, width: float, invert: bool = True, disp: bool = False, denoising_method: str = 'gaussian filter', thresholding_method: str = 'otsu'):
    ...
    Args:
        ...
        invert (bool): boolean value of whether or not image should be inverted
...
def blob_detection(img: np.ndarray, width: float, method: str = 'log', mol_min_size: float = CO_MIN_SIZE, mol_max_size: float = CO_MAX_SIZE, disp: bool = False):
