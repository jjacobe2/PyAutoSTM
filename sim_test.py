...
def automation_main(molecule_R, final_R, centX, centY, width, height):
    ...
    for i in np.arange(stm_img.assigned_init_config):
        ...
        pixel_imol_loc = stm_img.point2pixel(stm_img.assigned_init_config[i, :])
        pixel_path_arr_starttoi = astar.find_path_array(stm_img.raw_image, 1, pixel_start, pixel_imol_loc)
        ...
        pixel_fmol_loc = stm_img.point2pixel(stm_img.assigned_final_config[i, :])
        pixel_path_arr_itof = astar.find_path_array(stm_img.raw_image, 1, pixel_imol_loc, pixel_fmol_loc)
